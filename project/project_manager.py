        ...
        if os.path.exists(self.annotations_file):
            with open(self.annotations_file, 'r', encoding='utf-8') as f:
                try:
                    ...
                    # Если файл повреждён, считаем его пустым
                    data = {}
            if not self._validate_data(data):
                # Если структура неверна, предлагаем пользователю выбор через главное окно,
                # но здесь мы просто сбросим в пустое состояние.
                # Фактически проверка должна вызываться до вызова load() в main_window.
                self.annotations = {}
                self.class_colors = {}
